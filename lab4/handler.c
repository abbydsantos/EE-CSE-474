...
void adc_handler(int whatever) {
  ...
  int pipefd;
  mkfifo(pwmpipe, 0666);
  pipefd = open(pwmpipe, O_WRONLY);
  if (pipefd == -1) {
    printf("failed to open pwmpipe\n");
    exit(0);
  }
  sprintf(buf, "%d", meanFront);
...
int readADC(unsigned int pin) {
  ...
  if (pin == 1) {
    fd = open("/sys/devices/ocp.3/helper.15/AIN4", O_RDONLY);
  } else if (pin == 2) {
    fd = open("/sys/devices/ocp.3/helper.15/AIN6", O_RDONLY);
  } else if (pin == 3) {
    fd = open("/sys/devices/ocp.3/helper.15/AIN2", O_RDONLY);
  } else {
    fd = open("/sys/devices/ocp.3/helper.15/AIN0", O_RDONLY);
  }
